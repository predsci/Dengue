...
plot(x = frcst.rel2[1, , 1], y = 1:nstates, col = colmat[1, 1], xlim = range(frcst.rel2, na.rm = TRUE), ylim = c(1, nstates1), type = "p", pch = 19, ylab = "", xlab = expression("MAE(ARIMA)/MAE(NULL)"), 
	yaxt = "n", log = "x")
for (i in 1:nhorizon) {
	for (j in 1:narima) {
...
for (i in 1:nhorizon) {
	...
	plot(x = frcst.rel2[1, , 1], y = 1:nstates, col = colmat[1, 1], xlim = c(xmin, xmax), ylim = c(1, nstates1), type = "p", pch = 19, ylab = "", xlab = xlab, yaxt = "n", log = "x")
	...
	for (j in 1:narima) {
		...
		lines(x = frcst.rel2[i, , j], y = 1:nstates, col = colmat[j, 1], type = "p", pch = 19, ylab = "", xlab = "", cex = cex[1])
	...
	legend = paste0(mean.arima2)
	par(font = 2)
	ncol = 1
	if (i == 1) ncol = 2
	legend("bottomright", legend = legend, text.col = colmat[, 1], bty = "n", cex = 1, ncol = ncol)
}
...
plot(x = nfrcst.mae[1,1 , 1], y = 1, col = colmat[1, 1], xlim = range(nfrcst.mae, na.rm = TRUE), ylim = c(1, 1.33), type = "p", pch = 19, ylab = "", xlab = expression("MAE of ARIMA Models"), 
	yaxt = "n", log = "x")
for (i in 1:nhorizon) {
	for (j in 1:narima) lines(x = nfrcst.mae[i,1 , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 19, ylab = "", xlab = "", cex = 1.5)
	for (j in 1:narima) lines(x = nfrcst.mae[i,2 , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 17, ylab = "", xlab = "", cex = 1.5)
}

abline(h = (1 + (1:4 - 1) * 0.1), col = alpha("grey", 0.3))
axis(2, at = (1 + (1:4 - 1) * 0.1), label = paste0(1:4,'-month'), cex.axis = 1, las = 2)


legend = expression(arima.names[12])
legend("topleft", legend = paste0(arima.names, 12), text.col = colvec, bty = "n", ncol = 1)
...
plot(x = nfrcst.mre[1,"Direct" , 1], y = 1, col = colmat[1, 1], xlim = range(nfrcst.mre, na.rm = TRUE), ylim = c(1, 1.33), type = "p", pch = 19, ylab = "", xlab = expression("MRAE of ARIMA Models"), 
	yaxt = "n", main = tit, log = "x")
for (i in 1:nhorizon) {
	for (j in 1:narima) lines(x = nfrcst.mre[i,"Direct" , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 19, ylab = "", xlab = "", cex = 1.5)
	for (j in 1:narima) lines(x = nfrcst.mre[i,"Aggregate" , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 17, ylab = "", xlab = "", cex = 1.5)
}
abline(h = (1 + (1:4 - 1) * 0.1), col = alpha("grey", 0.3))
axis(2, at = (1 + (1:4 - 1) * 0.1), label = paste0(1:4,'-month'), cex.axis = 1, las = 2)

xmin = round(min(nfrcst.rel2, na.rm = TRUE), digits = 2)
xmax = round(max(nfrcst.rel2, na.rm = TRUE), digits = 2)
...
plot(x = nfrcst.rel2[1, 1, 1], y = 1, col = colmat[1, 1], xlim = range(nfrcst.rel2, na.rm = TRUE), ylim = c(1, 1.33), type = "p", pch = 19, ylab = "", xlab = expression("MAE(ARIMA)/MAE(NULL)"), 
	yaxt = "n", log = "x")
for (i in 1:nhorizon) {

	for (j in 1:narima) lines(x = nfrcst.rel2[i,1 , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 19, ylab = "", xlab = "", cex = 1.5)
	for (j in 1:narima) lines(x = nfrcst.rel2[i,"Aggregate" , j], y = (1 + (i - 1) * 0.1), col = colmat[j, i], type = "p", pch = 17, ylab = "", xlab = "", cex = 1.5)
...
filename = paste0(country, ".natl.arima.lines.pdf")

pdf(file = filename, width = 11, height = 7)
...
plot(x=1:nhorizon, y = nfrcst.mae[ ,"Direct" , 1], col = colmat[1, 1], xlim = c(1,nhorizon), ylim = range(nfrcst.mae), ylab = expression("MAE of ARIMA Models"), log = 'y', xlab = 'Forecast horizon', xaxt='n', type = 'l')

for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.mae[,"Direct" , j], col = colmat[j, 1], type = "l", pch = 19, ylab = "", xlab = "", cex = 1.5)
for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.mae[,"Aggregate" , j], col = colmat[j, 1], type = "l", lty = 2, ylab = "", xlab = "", cex = 1.5)
axis(1, at = 1:nhorizon, label = paste(1:nhorizon, ' month'))


legend = expression(arima.names[12])
legend("topright", legend = paste0(arima.names, 12), text.col = colvec, bty = "n", ncol = 1)

tit = "" #paste0("NATIONAL LEVEL ARIMA ANALYSYS: ", country.name)

plot(x=1:nhorizon, y = nfrcst.mre[ ,"Direct" , 1], col = colmat[1, 1], xlim = c(1,nhorizon), ylim = range(nfrcst.mre), ylab = expression("MRAE of ARIMA Models"), log = 'y', xlab = 'Forecast horizon', xaxt='n', type = 'l')

	for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.mre[,"Direct" , j], col = colmat[j, 1], type = "l",          ylab = "", xlab = "", cex = 1.5)
	for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.mre[,"Aggregate" , j], col = colmat[j, 1], type = "l", lty = 2, ylab = "", xlab = "", cex = 1.5)
...
plot(x=1:nhorizon, y = nfrcst.rel2[ ,"Direct" , 1], col = colmat[1, 1], xlim = c(1,nhorizon), ylim = range(nfrcst.rel2), ylab = expression("MAE(ARIMA)/MAE(NULL)"), log = 'y', xlab = 'Forecast horizon', xaxt='n', type = 'l')

for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.rel2[,"Direct" , j], col = colmat[j, 1], type = "l",          ylab = "", xlab = "", cex = 1.5)
for (j in 1:narima) lines(x=1:nhorizon, y = nfrcst.rel2[,"Aggregate" , j], col = colmat[j, 1], type = "l", lty = 2, ylab = "", xlab = "", cex = 1.5)

axis(1, at = 1:nhorizon, label = paste(1:nhorizon, ' month'))

abline(h = 1, col = alpha("grey", 0.8), lty = 1, lwd = 2)
